...
(function (LoaderState) {
    LoaderState[LoaderState["FadeIn"] = 0] = "FadeIn";
    LoaderState[LoaderState["Show"] = 1] = "Show";
    LoaderState[LoaderState["FadeOut"] = 2] = "FadeOut";
...
var PreloaderWeb = (function () {
     ...
     * Public
     */
    PreloaderWeb.prototype.init = function () {
        window['fullscreenContainer'] = document.getElementsByClassName('webpreloader__fullscreen-container')[0];
        this.gameContainer = document.getElementsByClassName('webpreloader__game-container')[0];
        this.wrapper = document.getElementsByClassName('webpreloader__wrapper')[0];
        this.slide = document.getElementsByClassName('webpreloader__animation')[0];
        this.progressBarFill = document.getElementsByClassName('webpreloader__progress-bar')[0];
        this.sliderValue = document.getElementsByClassName('webpreloader__progress-value')[0];
        this.animationContainers = [];
        this.animationContainers.push(document.getElementsByClassName('webpreloader__header')[0]);
        var icons = document.getElementsByClassName('webpreloader__icon');
        var labels = document.getElementsByClassName('webpreloader__skill-description');
        for (var i = 0; i < icons.length; i++) {
            this.animationContainers.push(icons[i]);
            this.animationContainers.push(labels[i]);
        }
        this.animationContainers.push(document.getElementsByClassName('webpreloader__animation')[0]);
     ...
     */
    PreloaderWeb.prototype.setProgress = function (value, isUnityProgress, isInstant) {
        ...
        var preloadValue = .5 * value;
        if (isUnityProgress) {
            preloadValue = value;
        }
        this.progressBarTargetValue = preloadValue;
    };
    PreloaderWeb.prototype.show = function () {
        var _this = this;
        if (this.isVisible)
            return;
        ...
        this.progressBarFill.style.left = (this.progressBarValue - 1) * LoaderConf.progressBarFullWidth + "px";
        this.progressBarTargetValue = 0;
        this.checkWebpSupport(function (isSupported) {
            _this.isWebPSupported = isSupported;
            _this.showNextImage();
            _this.update();
        });
    };
     ...
     * @param {boolean} isUnityProgress - is function called from unity plugin
     */
    PreloaderWeb.prototype.hide = function (isUnityProgress) {
        if (isUnityProgress) {
            if (this.gameContainer)
    ...
    PreloaderWeb.prototype.updateSize = function () {
    };
    PreloaderWeb.prototype.showLoading = function () {
    };
    PreloaderWeb.prototype.hideLoading = function () {
    };
    PreloaderWeb.prototype.getCurrentImageId = function () {
        return this.gameErrorSlideID;
    };
    /**
     * Private
     */
    PreloaderWeb.prototype.update = function () {
    ...
    PreloaderWeb.prototype.updateBarState = function (delta) {
    ...
    PreloaderWeb.prototype.updateAnimationState = function (delta) {
        ...
        if (this.currentState === LoaderState.Show) {
            var isNextSlideLoaded = this.preloadedCount == LoaderConf.preloadImagesCount;
            var isShowTimePassed = Date.now() - this.showTimestamp >= LoaderConf.showTime;
            //prevent slide switch on unity initialization
            var isLagZone = Math.abs(this.progressBarTargetValue - .5) < 0.05;
    ...
    PreloaderWeb.prototype.showNextImage = function () {
        var _this = this;
        this.currentImageID = this.preloadedImageID;
        if (this.slide != null) {
            this.slide.style.backgroundImage = "url(" + this.getImageURL(this.currentImageID) + ")";
        }
        var header = document.getElementsByClassName('webpreloader__header')[0];
        header.textContent = this.locale[this.preloadedImageID][0];
        var rows = document.getElementsByClassName('webpreloader__skill');
        var iconExtension = ".png";
        for (var index = 0; index < rows.length; index++) {
            var item = rows[index];
            var description = item.getElementsByClassName('webpreloader__skill-description')[0];
            description.textContent = this.locale[this.preloadedImageID][index + 1];
            var icon = item.getElementsByClassName('webpreloader__icon')[0];
        ...
        this.preloadedImages = [];
        this.preloadedCount = 0;
        var preloadedSlideID = this.preloadedImageID;
        var preloadedSlideURL = this.getImageURL(preloadedSlideID);
        var preloadedSlide = new Image();
        preloadedSlide.src = preloadedSlideURL;
        preloadedSlide.onload = function () {
            _this.gameErrorSlideID = preloadedSlideID;
            _this.preloadedCount++;
        };
        preloadedSlide.onerror = function () {
            _this.preloadedCount++;
        };
        ...
        preloadURLs.forEach(function (val) {
            var preloadedImage = new Image();
            preloadedImage.src = val;
            preloadedImage.onload = preloadedImage.onerror = function () {
                _this.preloadedCount++;
            };
            _this.preloadedImages.push(preloadedImage);
        });
    };
    PreloaderWeb.prototype.getImageExtension = function () {
        return ".jpg";
        // return this.isWebPSupported ? ".webp" : ".jpg";
    };
    PreloaderWeb.prototype.getImageURL = function (id) {
        return LoaderConf.slidePath + id + "/bg" + ".jpg";
    };
    PreloaderWeb.prototype.checkWebpSupport = function (callback) {
        var webP = new Image();
    ...
    PreloaderWeb.prototype.updateOpacity = function () {
        var opacityValue = this.opacityLevel.toString();
        for (var i = 0; i < this.animationContainers.length; i++) {
            this.animationContainers[i].style.opacity = opacityValue;
        }
    };
    PreloaderWeb.prototype.updateLocale = function () {
        var lang = PreloaderWeb.getLanguage();
        var locale = getLocale();
        this.locale = locale[lang] ? locale[lang] : locale["en"];
        var longLoadingText = getLongLoadingText();
        var longLoadingTextContainer = document.getElementsByClassName('webpreloader__tips-text')[0];
    ...
    PreloaderWeb.getLanguage = function () {
        var integrationHelper = window['integrationHelper'];
        if (!integrationHelper)
            console.error("intergation helper not loaded");
        var lang = localStorage.getItem("webpreloaderLanguage");
        if (!lang) {
            if (integrationHelper)
...
var preloader = new PreloaderWeb();
var webPreloaderSetProgress = function (value, isInstant) {
    preloader.setProgress(value, true, Boolean(isInstant).valueOf());
};
var webPreloaderHide = function () {
    preloader.hide(true);
};
var webPreloaderShow = function () {
    preloader.show();
};
var webPreloaderGetCurrentImageId = function () {
    return preloader.getCurrentImageId();
};
var webPreloaderSetPreloaderLanguage = function (lang) {
